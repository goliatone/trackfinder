...
TrackFinder.prototype.find = function(path) {
    ...
    this.logger.debug('trackfinder directory: "%s"', directory);
...
TrackFinder.prototype.register = function(app, options) {
    ...
    this.logger.debug('LOAD FILES: %s', routeFiles);

    var router, args, routers = [];
    routeFiles.forEach(function(route) {
        args = [];
        router = express.Router();
        this.logger.debug('route: %s', route._ref);
        ...
        if (route.routes) {
            ...
            Object.keys(route.routes).forEach(function(key) {
                ...
                parameters = bits[1];
                this.logger.debug('verb %s, parameters %s', verb, parameters);
